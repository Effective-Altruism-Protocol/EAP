...
const checkIfAccountChanged = async () => {
  ...
  */
  const renderRegisterFoundation = () => {
    if (walletConnected) {
      if (foundationName) {
        return <div className={styles.description}>{foundationName}</div>;
      } else if (loading) {
        ...
        return (
          <div>
            ...
            <button onClick={addFoundation} className={styles.button}>
              Register Foundation
            </button>
          </div>
        );
  ...
  return (
    <div>
      ...
      <main className="flex flex-column align-items-center">
        <div className=" w-5">
          <h1 className={styles.title}>{!foundationName ? "Please Register a Foundation" : "Welcome"}</h1>
